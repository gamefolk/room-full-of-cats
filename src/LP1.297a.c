/*
   ...
   you can change inside the "void sound ()" part the number of music you want
   to play (don't forget to place the same value in the song_def.c and the
   exemple.bat)

   modified on 26 aug 2000
   found a bug in volume update, that trashed the frequencies in voice 1 and 2
   on real GB
...
/* Include the file that will say to the player what it needs to play */
#include "music/songs_def.c"
...
unsigned int snd_lngh0;     /* to play different instruments on one track */
 ...
 * To create 1 new instrument, just use the GameBee Synthetiser, and save it
 * for RGBDS file, then transform the line to be like those... et voila
 */
...
void effect_test() {
    ...
    if (((data_song >> 4) & 0x0003) == 1) {
        ...
        if (voice == 2) {
            ...
            if (fx_value >= 1 && fx_value <= 5) {
                fx_value = 0x60U;
            }
            if (fx_value >= 6 && fx_value <= 10) {
                fx_value = 0x40U;
            }
            if (fx_value >= 11 && fx_value <= 15) {
                fx_value = 0x20U;
            }
            fx_volume = fx_value;
            if (gb_freq == 0x48) {
...
void instru_test() {
    if (voice == 0) {
        if (((data_song >> 6) & 0x0007) == 0) {
            /* instrument 1 with sustain middle */
            snd_swp0 = 0x00U;
            snd_lngh0 = 0x07U;
            snd_env0 = fx_volume | 0x02U;
            snd_cons0 = 0x80U;
        } else if (((data_song >> 6) & 0x0007) == 1) {
            /* instrument 2 with sustain middle */
            snd_swp0 = 0x00U;
            snd_lngh0 = 0x47U;
            snd_env0 = fx_volume | 0x02U;
            snd_cons0 = 0x80U;
        } else if (((data_song >> 6) & 0x0007) == 2) {
            /* instrument 3 with sustain middle */
            snd_swp0 = 0x00U;
            snd_lngh0 = 0x87U;
            snd_env0 = fx_volume | 0x02U;
            snd_cons0 = 0x80U;
        } else if (((data_song >> 6) & 0x0007) == 3) {
            /* same as 1 with sustain infinite */
            snd_swp0 = 0x00U;
            snd_lngh0 = 0x07U;
        ...
        if (((data_song >> 6) & 0x0007) == 4) {
            /* same as 2 with sustain infinite */
            snd_swp0 = 0x00U;
            snd_lngh0 = 0x47U;
            snd_env0 = fx_volume | 0x00U;
            snd_cons0 = 0x80U;
        } else if (((data_song >> 6) & 0x0007) == 5) {
            /* same as 3 with sustain infinite */
            snd_swp0 = 0x00U;
            snd_lngh0 = 0x87U;
            snd_env0 = fx_volume | 0x00U;
            snd_cons0 = 0x80U;
        } else if (((data_song >> 6) & 0x0007) == 6) {
            /* create your instrument 7 here */
            snd_swp0 = 0x00U;
            snd_lngh0 = 0x07U;
            snd_env0 = fx_volume | 0x07U;
            snd_cons0 = 0x80U;
        } else if (((data_song >> 6) & 0x0007) == 7) {
            /* create your instrument 8 here */
            snd_swp0 = 0x00U;
            snd_lngh0 = 0x07U;
            snd_env0 = fx_volume | 0x07U;
            snd_cons0 = 0x80U;
...
void voice0() {
    ...
    if (gb_freq != 0x48) {
        ...
        NR13_REG = freqLOW;                       /* 8 bits of data */
        NR14_REG = snd_cons0 | freqHI;            /* 3 bits of data */
...
/*
 * I'm sure there is plenty of better way to do this but don't blame me I'm not
 * god ;o)
 */
